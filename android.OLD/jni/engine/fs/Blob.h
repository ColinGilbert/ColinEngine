...
{
public:
	blob(): file_data( nullptr ), file_owns_data( true ), file_size( 0 ), file_current_pos( 0 ), allocated_size( 0 ) {}
	...
	bool has_data() const  { return ( file_data != nullptr ); }
	...
	*/
	int compare_contents( const weak_ptr<blob>& other ) const
	{
	...
	bool copy_memoryblock( const size_t* from_data, size_t from_size )
	{
...
private:
	...
	{
		if ( !file_owns_data || file_data == nullptr ) { return; }
...
public:
	...
	void read_bytes( unsigned char* out, size_t num ) { get_bytes( file_current_pos, num, out ); file_current_pos += num; }
