...
refcounting_ptr<input_stream> filesystem::create_reader( const std::string& filename ) const
{
	std::string name = arch_fix_filename( filename );

	refcounting_ptr<mountpoint> mountpoint = find_mountpoint( name );
	refcounting_ptr<raw_file> RAWFile = mountpoint->create_reader( name );
...
refcounting_ptr<input_stream> filesystem::reader_from_string( const std::string& str ) const
{
...
refcounting_ptr<input_stream> filesystem::reader_from_memory( const void* buffer_size, uint64_t bufsize, bool owns_data ) const
{
	mem_rawfile* raw_file = new mem_rawfile();
...
refcounting_ptr<input_stream> filesystem::reader_from_blob( const refcounting_ptr<clBlob>& Blob ) const
{
...
bool filesystem::file_exists( const std::string& name ) const
{
	...
	refcounting_ptr<mountpoint> mount_point_dir = find_mountpoint( name );
...
std::string filesystem::virtual_name_to_physical( const std::string& path ) const
{
	...
	refcounting_ptr<mountpoint> MP = find_mountpoint( path );
	return ( !MP ) ? path : MP->map_name( path );
}

void filesystem::mount( const std::string& physical_path )
{
	refcounting_ptr<mountpoint> mount_point_dir = nullptr;
...
{
	refcounting_ptr<mountpoint> MP = find_mountpoint_by_name( src_path );
...
refcounting_ptr<mountpoint> filesystem::find_mountpoint_by_name( const std::string& path_arg )
{
...
void filesystem::add_mountpoint( const refcounting_ptr<mountpoint>& MP )
{
	if ( !MP ) { return; }
...
refcounting_ptr<mountpoint> filesystem::find_mountpoint( const std::string& filename ) const
{
	...
	// reverse order
	for ( std::vector<refcounting_ptr<mountpoint> >::const_reverse_iterator i = mountpoints.rbegin(); i != mountpoints.rend(); ++i )
